...
class HomeSwipeViewController: UIViewController, KolodaViewDataSource, KolodaViewDelegate, UISearchBarDelegate {
    ...
    func searchBarTextDidBeginEditing(searchBar: UISearchBar) {
        searchActive = true;
        println("User started editing text")
    }
    
    func searchBarTextDidEndEditing(searchBar: UISearchBar) {
        searchActive = false;
        println("User finished editing text")
    }
    ...
    func searchBarSearchButtonClicked(searchBar: UISearchBar) {
        ...
        searchBar.text = ""
        searchBar.endEditing(true)
        println("User clicked search button to search for \(searchString)")
        searchDisplayOverview.hidden = true
    }
    ...
    /*
    @IBAction func userDidEnterSearchField(sender: AnyObject) {
        if searchField.text.isEmpty {
            // nothing entered
        } else {
            // pass the value to the sarch parameter
            var searchCriteria = searchField.text
            println(searchCriteria)
            // close teh search field
            searchDisplayOverview.hidden = !searchDisplayOverview.hidden
        }
    }*/
    ...
    func kolodaDidSwipedCardAtIndex(koloda: KolodaView, index: UInt, direction: SwipeResultDirection) {
        ...
        if direction == SwipeResultDirection.Right {
            println("User swiped right, save to favorites")
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        let favoritesTVC = segue.destinationViewController as! FavoritesTVController
            println("There are \(favoriteRestaurants.count) favorites")
